...
export default class NovelDetail extends React.Component {
  ...
  viewMap = () => {
    ...
    const locate = this.props.navigation.getParam("Route", "NO-ID").FirstLocate.split(',');
    const latLng = `${locate[0]},${locate[1]}`;
    const label = this.props.navigation.getParam("Route", "NO-ID").FirstLocateName;
          ...
          >
            <View style={{ height: "100%", }}>
              <View style={{ height: 40, width: "100%", flexDirection: "row" }}>
                <Icon name="car" size="lg" color="#2ae8bf" />
                <Text style={{ fontSize: 21, color: "#2ae8bf" }}>
                  {" "}
                  Bus information
                </Text>
              </View>
              ...
              <View style={{ padding:10, borderBottomWidth: 1, borderBottomColor:'#ccc'}}>
                ...
                <Text style={{fontSize: 18}}>{currentItem.FirstLocateName} - {currentItem.EndLocateName}</Text>
              </View>

              <View style={{ margin:10, borderBottomColor:'#ccc'}}>
                <Text style={{fontSize: 18, fontWeight:'bold'}}>{moment(currentItem.DepartureTime).format('HH:mm')}</Text>
                <Text style={{fontSize: 18}}>{currentItem.FirstLocateName}</Text>
                <Text style={{fontSize: 18}}>Parking description: {currentItem.ParkingLot}</Text>
                <Text style={{fontSize: 18}}>Parking fee: {currentItem.ParkingFee} vnd</Text>
              </View>
              <View style={{width:'100%', justifyContent:'center', alignContent:'center', flexDirection:'row'}}>
                 <Icon style={{margin:'auto'}} name="arrow-down" size="lg" color="#2ae8bf" />
              </View>
              <View style={{ padding:10, borderBottomWidth: 1,borderBottomColor:'#ccc'}}>
                <Text style={{fontSize: 18, fontWeight:'bold'}}>{moment(currentItem.ArriveTime).format('HH:mm')}</Text>
                <Text style={{fontSize: 18}}>{currentItem.EndLocateName}</Text>
              </View>

              <View style={{ padding:10, borderBottomWidth: 1,borderBottomColor:'#ccc'}}>
                <Text style={{fontSize: 18, fontWeight:'bold'}}>{`17:30`}</Text>
                <Text style={{fontSize: 18}}>Departure Time in the afternoon  </Text>
              </View>
