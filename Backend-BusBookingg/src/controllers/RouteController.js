...
const getAll = `SELECT bus_route.Id, bus_route.IsEnable,bus_route.Name, BusId,bus.Number as 'busNumber',bus_type.SeatNumber as 'maxSeat', bus_type.Name as 'busType',user.Account as 'driverAccount', 
...
module.exports = {
  ...
  getRouteById: async (req, res) => {
    ...
    let sql = getByRouteId;
    db.query(sql, [req.params.id], async (err, response) => {
      if (err) throw err;
      response[0].locates = await getLocates(req.params.id)
      res.json(response[0]);
    });
  },
  update: async (req, res) => {
    ...
    db.query(sql, [{...data, locates: 0}, Id], async (err, response) => {
      if (err) throw err;
      await clearLocates(Id)
      console.log(data.locates);
      if (data.locates && Array.isArray(data.locates)) {
        await Promise.all(
          data.locates.map(async (item) => {
            await setLocate({ ...item, routeId: Id },user.Id);
          })
        );
      }
  ...
  store: async (req, res) => {
    ...
    db.query(sql, [{...data, locates: 0}], async (err, response) => {
      if (err) throw err;
     // set locates
      if (data.locates && Array.isArray(data.locates)) {
        await Promise.all(
          data.locates.map(async (item) => {
            await setLocate({ ...item, routeId: response.insertId },user.Id);
          })
        );
      }
...
const getLocates = async (routeId) =>
  new Promise((resolve, reject) => {
...
const setLocate = async (data, userId) =>
  new Promise((resolve, reject) => {
    let sql = `INSERT INTO route_locate SET ?`;
    const result = _.pick(data, ["locateId", "routeId", "ArriveTime"]);
    result.modifiedOn = new Date().toISOString();
    result.modifiedBy = userId;

    db.query(sql, [result], (err, response) => {
      if (err) throw err;
      resolve(response);
    });
  });

const clearLocates = async (routeId) =>
  new Promise((resolve, reject) => {
    let sql = `DELETE FROM route_locate WHERE routeId = ?`;
    db.query(sql, [routeId], (err, response) => {
      if (err) throw err;
      resolve(response);
    });
